...
# /home/ja/.local/share/ov/pkg/isaac-sim-4.0.0/python.sh ./code/simulation/object_vis.py 
...
config = {
    ...
    "env_url": "",
    "working_area_size": (0.7, 0.7, 1.5),
    "generation_area_size":(0.5, 0.5, 0.5),
    "rt_subframes": 4,
    "num_frames": 1,
    "num_cameras": 1,
    ...
    "camera_distance_to_target_min_max": (0.5, 1),
    "writer_type": "BasicWriter",
    "writer_kwargs": {
        # "output_dir": DATA_FOLDER / "camera",
        "rgb": True,
        # "instance_segmentation": True,
        # "distance_to_image_plane": True,
        "use_common_output_dir": True,
    },
    "save_path": DATA_FOLDER / "obj_vis/camera",
    "table_url": "/Isaac/Props/Mounts/SeattleLabTable/table_instanceable.usd",
    "table_pos": (0,0, - 0.4),
    "random_object_info": CONFIG_FOLDER / "object_info.csv",
    # "num_object_category": 10,
    "min_obj_num":1,
    "max_obj_num":1,
    "random_table_info": CONFIG_FOLDER / "table_info.csv",
    "start_from":0,
    "end_with": 190,
}
...
# start of the scene loop
for scene_num in range(start_from, end_with):
    ...
    # Get the working area size and bounds (width=x, depth=y, height=z) #lzz this mark is used for the ease of modefication
    ...
    # working_area_min = (working_area_size[0] / -2, working_area_size[1] / -2, 0)
    ...
    [random_table] = random_choose_object.get_random_table_from_file(random_table_info)
    # table_url = config.get("table_url", None)
    print("random_table",random_table)
    table_url = random_table['path']
    if table_url == None:
        ...
        # object_based_sdg_utils.set_transform_attributes(prim, location=table_pos)
    ...
    obj = random_choose_object.get_target_object_from_file(random_object_info, scene_num)
    ...
    camera_properties_kwargs = config.get("camera_properties_kwargs", {}) #lzz: check the properties
    ...
    writer_kwargs = config.get("writer_kwargs", {}) #lzz: check
    ...
    # physx_sub = get_physx_interface().subscribe_physics_step_events(on_physics_step)#lzz
    ...
    def save_camera_matrices(cam, resolution, scene_num, i, DATA_FOLDER):
        ...
        output_path = DATA_FOLDER / "obj_vis/camera" / str(scene_num) / file_name
    ...
    def randomize_camera_poses():#lzz : check
        for cam in cameras:
    ...
    with rep.trigger.on_custom_event(event_name="randomize_lights"):
        lights = rep.create.light(
            ...
            # position=rep.distribution.uniform(geration_area_min, geration_area_max),
    ...
    num_frames = config.get("num_frames", 10)#lzz
    ...
    # for cam in cameras:    
    #     print("------------camera-----------")
    #     print("focalLength",cam.GetAttribute("focalLength").Get())
    #     focal_length = cam.GetAttribute("focalLength").Get()
    #     print("horizontalAperture",cam.GetAttribute("horizontalAperture").Get())
    #     horiz_aperture = cam.GetAttribute("horizontalAperture").Get()
    ...
    #     print("------------camera-----------")
    #     position = cam.GetAttribute("xformOp:translate").Get()
    #     rotation = cam.GetAttribute("xformOp:rotateXYZ").Get()
    #     print("position",position)
    #     print("rotation",rotation)
    #     print("------------camera-----------")

    output_usd_path = os.path.join(config.get("save_path"), str(scene_num), "scene/scene.usd")
    # output_usd_path = str(DATA_FOLDER / "camera" / str(scene_num) / "scene/scene.usd")
    # # print("?????????????")
    try:
